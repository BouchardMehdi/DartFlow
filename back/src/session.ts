...
export interface SessionUser { id: string; email: string; username: string; avatar?: string; createdAt: string; sessionVersion: number }
...
async function setCookies(reply: FastifyReply, user: SessionUser, refreshToken: string) {
  ...
  const access = await reply.jwtSign({ email: user.email, username: user.username, ver: user.sessionVersion }, { sign: { sub: user.id, expiresIn: `${config.ACCESS_TOKEN_TTL_HOURS}h` } });
...
export async function rotateSession(reply: FastifyReply, refreshToken: string | undefined, userAgent?: string) {
  ...
  try {
    ...
    const result = await client.query<{ id: string; user_id: string; revoked_at: Date | null; expires_at: Date; email: string; username: string; avatar: string | null; created_at: Date; session_version: number }>(`SELECT rs.id,rs.user_id,rs.revoked_at,rs.expires_at,u.email,u.username,u.avatar,u.created_at,u.session_version FROM refresh_sessions rs JOIN users u ON u.id=rs.user_id WHERE rs.token_hash=$1 FOR UPDATE`, [hash(refreshToken)]);
    ...
    const user = { id: current.user_id, email: current.email, username: current.username, ...(current.avatar ? { avatar: current.avatar } : {}), createdAt: current.created_at.toISOString(), sessionVersion: current.session_version };
