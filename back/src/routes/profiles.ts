...
import { normalizeEmail, normalizeName } from "../domain.js";
...
const shareSchema = z.object({ email: z.email().max(254), role: z.enum(["editor", "viewer"]) });
...
const profileRoutes: FastifyPluginAsync = async (app) => {
  app.patch("/:id", { preHandler: app.authenticate }, async (request, reply) => {
    ...
    if (!role || role === "viewer") return reply.code(403).send({ message: "Vous ne pouvez pas modifier ce profil." });
  ...
  app.get("/:id/shares", { preHandler: app.authenticate }, async (request, reply) => {
    ...
    const [members, invites] = await Promise.all([
      pool.query("SELECT u.id user_id, u.email, pa.role FROM profile_access pa JOIN users u ON u.id=pa.user_id WHERE pa.profile_id=$1 ORDER BY u.email", [params.data.id]),
      pool.query("SELECT email, role FROM share_invitations WHERE profile_id=$1 ORDER BY email", [params.data.id]),
    ]);
    return { shares: members.rows.map((row) => ({ userId: row.user_id, email: row.email, role: row.role })), invitations: invites.rows };
  });
  ...
  app.post("/:id/shares", { preHandler: app.authenticate }, async (request, reply) => {
    ...
    if (email === request.user.email) return reply.code(400).send({ message: "Vous êtes déjà propriétaire de ce profil." });
    const target = await pool.query<{ id: string }>("SELECT id FROM users WHERE email=$1", [email]);
    if (target.rows[0]) {
      await pool.query("INSERT INTO profile_access(profile_id,user_id,role) VALUES($1,$2,$3) ON CONFLICT(profile_id,user_id) DO UPDATE SET role=EXCLUDED.role", [params.data.id, target.rows[0].id, body.data.role]);
      return { status: "shared" };
    }
    await pool.query("INSERT INTO share_invitations(id,profile_id,email,role) VALUES($1,$2,$3,$4) ON CONFLICT(profile_id,email) DO UPDATE SET role=EXCLUDED.role", [randomUUID(), params.data.id, email, body.data.role]);
