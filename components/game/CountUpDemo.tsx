  ...
  return (
    <main className="mx-auto min-h-screen max-w-6xl px-4 pb-10 pt-5 sm:px-7">
      <header className="mb-6 flex flex-col gap-3 border-b border-[var(--line)] pb-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3"><span className="grid size-9 place-items-center rounded-full bg-[var(--lime)] text-xl font-black text-black">↗</span><div><p className="text-lg font-black tracking-[-.04em]">DARTFLOW</p><p className="text-[10px] uppercase tracking-[.24em] text-[var(--muted)]">{modeState.kind === "count-up" ? "Count‑Up" : modeState.kind === "x01" ? `${modeState.startingScore} · ${modeState.exitRule} out · ${modeState.legsToWin}L/${modeState.setsToWin}S` : modeState.kind === "around-the-clock" ? "Around the Clock" : modeState.kind === "shanghai" ? "Shanghai" : modeState.kind === "cricket" ? modeState.variant === "standard" ? "Cricket standard" : modeState.variant === "no-score" ? "Cricket sans points" : "Cut-Throat Cricket" : "Killer"}</p></div></div>
      ...
      <AnimatePresence>{winner && <FinalResults game={game} onReplay={() => { if (game.modeState.kind === "count-up") useGameStore.getState().start(game.players, game.modeState.maxRounds); else if (game.modeState.kind === "x01") useGameStore.getState().startX01(game.players, game.modeState.startingScore, game.modeState.entryRule, game.modeState.exitRule, game.modeState.maxRounds, game.modeState.legsToWin, game.modeState.setsToWin); else if (game.modeState.kind === "around-the-clock") useGameStore.getState().startAroundTheClock(game.players, game.modeState.progressionRule, game.modeState.bullFinish, game.modeState.maxRounds, game.modeState.direction); else if (game.modeState.kind === "shanghai") useGameStore.getState().startShanghai(game.players, game.modeState.maxRounds, game.modeState.instantShanghaiWin, game.modeState.startTarget); else if (game.modeState.kind === "cricket") useGameStore.getState().startCricket(game.players, game.modeState.maxRounds, game.modeState.variant); else useGameStore.getState().startKiller(game.players, game.modeState.startingLives, game.modeState.selfDamage, game.modeState.marksToKiller); }} />}</AnimatePresence>
    </main>
  );
}
