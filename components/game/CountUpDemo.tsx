...
export function CountUpDemo() {
  ...
  const playerScore = useCallback((playerId: string) => modeState.kind === "count-up" || modeState.kind === "shanghai" ? modeState.scores[playerId] ?? 0 : modeState.kind === "x01" || modeState.kind === "cricket" ? modeState.players[playerId]?.score ?? 0 : modeState.players[playerId]?.target ?? 1, [modeState]);
    ...
    <main className="mx-auto min-h-screen max-w-6xl px-4 pb-10 pt-5 sm:px-7">
      ...
      <section className="grid gap-5 lg:grid-cols-[minmax(19rem,1fr)_minmax(22rem,1.15fr)] lg:items-start">
        <div className="space-y-4 lg:sticky lg:top-5">
          ...
          {modeState.kind === "cricket" ? <CricketScoreboard state={modeState} players={game.players} activePlayerId={active?.id} /> : <div className="grid grid-cols-2 gap-3">
