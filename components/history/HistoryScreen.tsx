...
export function HistoryScreen() {
  ...
  return <main className="mx-auto min-h-screen max-w-5xl px-4 py-5 sm:px-7">
    ...
    <section className="mt-10"><p className="text-xs font-black uppercase tracking-[.18em] text-[var(--lime)]">Parties</p><h2 className="mt-2 text-3xl font-black tracking-[-.04em]">Derniers résultats</h2>
      {loading ? <p className="mt-5 text-[var(--muted)]">Chargement…</p> : games.length === 0 ? <p className="mt-5 rounded-2xl border border-[var(--line)] p-5 text-[var(--muted)]">Aucune partie terminée pour le moment.</p> : <div className="mt-5 space-y-3">{games.map((game) => { const winner = game.players.find((player) => player.id === game.winnerId); const mode = game.modeState.kind === "count-up" ? "Count‑Up" : game.modeState.kind === "x01" ? String(game.modeState.startingScore) : game.modeState.kind === "around-the-clock" ? "Around the Clock" : game.modeState.kind === "shanghai" ? "Shanghai" : game.modeState.kind === "cricket" ? "Cricket" : "Killer"; return <article key={game.id} className="flex items-center justify-between gap-4 rounded-2xl border border-[var(--line)] bg-[var(--panel)] p-4"><div className="min-w-0"><div className="flex items-center gap-2"><strong className="rounded-full bg-[var(--lime)]/10 px-2.5 py-1 text-xs text-[var(--lime)]">{mode}</strong><span className="text-xs text-[var(--muted)]">{new Date(game.completedAt ?? game.updatedAt).toLocaleDateString("fr-FR")} · {duration(game)}</span></div><p className="mt-2 truncate font-bold">Victoire de {winner?.name ?? "—"}</p><p className="mt-1 truncate text-sm text-[var(--muted)]">{game.players.map((player) => player.name).join(" · ")}</p></div><button type="button" onClick={() => void removeGame(game)} className="shrink-0 rounded-xl border border-[#713b32] px-3 py-2 text-xs font-bold text-[#ff9b7a]">Supprimer</button></article>; })}</div>}
    </section>
  </main>;
}
