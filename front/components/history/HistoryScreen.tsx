...
export function HistoryScreen() {
  ...
  const profileStats = useMemo(() => players.map((player) => {
    ...
    const matches = (item: { id: string; name: string }) => item.id === player.id || (!player.ownerUserId && normalizePlayerName(item.name) === normalizedName);
    const played = games.filter((game) => game.modeState.kind !== "training" && game.players.some(matches));
    const standings = played.flatMap((game) => { const matchingIds = new Set(game.players.filter(matches).map((item) => item.id)); return getFinalStandings(game).filter((standing) => matchingIds.has(standing.playerId)); });
    const wins = played.filter((game) => { const winner = game.players.find((item) => item.id === game.winnerId); return winner ? matches(winner) : false; }).length;
