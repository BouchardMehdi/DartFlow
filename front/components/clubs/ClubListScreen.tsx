...
export function ClubListScreen() {
  ...
  return <main className="mx-auto min-h-screen max-w-6xl px-4 py-8 sm:px-7">
    ...
    {discover.length>0&&<section className="mt-10"><h2 className="text-2xl font-black">Clubs publics</h2><div className="mt-4 grid gap-3 md:grid-cols-2 lg:grid-cols-3">{discover.map((club)=><article key={club.id} className="rounded-2xl border border-[var(--line)] bg-[var(--panel)] p-4"><div className="flex items-center gap-3"><div className="relative grid size-11 shrink-0 place-items-center overflow-hidden rounded-xl border border-[var(--line)] bg-black/25">{club.avatar?<Image src={club.avatar} alt="" fill sizes="44px" unoptimized className="object-cover"/>:<span className="font-black uppercase text-[var(--lime)]">{club.name.charAt(0)}</span>}</div><strong>{club.name}</strong></div><p className="mt-3 min-h-10 text-sm text-[var(--muted)]">{club.description}</p><div className="mt-4 flex items-center justify-between"><span className="text-xs text-[var(--muted)]">{club.memberCount} membres</span><button onClick={()=>void requestJoin(club.id)} className="rounded-lg border border-[var(--lime)] px-3 py-2 text-sm font-bold text-[var(--lime)]">Demander à rejoindre</button></div></article>)}</div></section>}
  </main>;
}
