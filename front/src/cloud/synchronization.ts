...
async function runSynchronization(user: AccountUser): Promise<string> {
  ...
  const request: SyncRequest = {
    ...
    games: games.filter((game) => !game.cloudUserId || game.cloudUserId === user.id).map((game) => ({ id: game.id, modeId: game.modeId, status: game.status, state: game, updatedAt: game.updatedAt })),
    ...
    await database.transaction("rw", database.players, database.games, database.syncOutbox, database.syncMetadata, async () => {
      for (const [oldId, newId] of Object.entries(response.profileIdMappings)) {
        ...
        for (const game of affectedGames) {
          ...
          if (parsed.success) await database.games.put({ ...parsed.data, savedAt: game.savedAt, cloudUserId: game.cloudUserId ?? user.id, ...(game.cloudVersion === undefined ? {} : { cloudVersion: game.cloudVersion }) } as SavedGame);
        }
      }
      for (const profile of response.profiles) {
        const local = await database.players.get(profile.id);
        const saved: SavedPlayer = { id: profile.id, name: profile.name, order: local?.order ?? 0, createdAt: profile.createdAt, updatedAt: profile.updatedAt, cloudUserId: user.id, cloudRole: profile.role, ownerEmail: profile.ownerEmail, isPublic: profile.isPublic, ...(profile.color ? { color: profile.color } : {}), ...(profile.avatar ? { avatar: profile.avatar } : {}) };
      ...
      for (const cloudGame of response.games) {
        ...
        if (parsed.success) await database.games.put({ ...parsed.data, savedAt: cloudGame.updatedAt, cloudUserId: user.id, cloudVersion: cloudGame.version } as SavedGame);
      }
      await database.syncOutbox.clear();
      await database.syncMetadata.put({ id: "main", lastSyncedAt: now });
    });
    return now;
  } catch (error) {
    await database.syncMetadata.put({ id: "main", lastError: error instanceof Error ? error.message : "Synchronisation impossible." });
    throw error;
  }
}
