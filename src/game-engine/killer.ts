...
export function createKillerGame(players: Player[], startingLives: number, random: () => number = Math.random, now = new Date()): GameState {
  ...
  const ordered = [...players].sort((a, b) => a.order - b.order); const first = ordered[0]; if (!first) throw new Error("Joueur manquant"); const numbers = assignKillerNumbers(ordered, random); const date = now.toISOString();
  return { id: crypto.randomUUID(), modeId: "killer", status: "in-progress", players: ordered, currentPlayerIndex: 0, currentRound: 1, currentTurn: makeTurn(first, 1, date), turns: [], modeState: { kind: "killer", startingLives, marksToKiller: 3, players: Object.fromEntries(ordered.map((player) => [player.id, { number: numbers[player.id] ?? 1, marks: 0, isKiller: false, lives: startingLives, eliminated: false }])) }, createdAt: date, updatedAt: date };
}
...
export function registerKillerThrow(state: GameState, dart: DartThrow, now = new Date()): EngineResult {
  ...
  if (dart.segment !== null && dart.multiplier > 0) {
    ...
    else if (attacker.isKiller && dart.segment !== attacker.number) { const victim = state.players.find((item) => { const candidate = playersState[item.id]; return candidate && !candidate.eliminated && candidate.number === dart.segment; }); if (victim) { const victimState = playersState[victim.id]; if (victimState) { const lives = Math.max(0, victimState.lives - dart.multiplier); damage = victimState.lives - lives; const eliminated = lives === 0; playersState[victim.id] = { ...victimState, lives, eliminated }; if (eliminated) events.push({ type: "PLAYER_ELIMINATED", playerId: victim.id }); } } }
  }
  const activePlayers = state.players.filter((item) => !playersState[item.id]?.eliminated); const won = activePlayers.length === 1; const darts = [...state.currentTurn.darts, dart]; const completed = darts.length === 3 || won; const turnScore = state.currentTurn.turnScore + damage; const turn: Turn = { ...state.currentTurn, darts, turnScore, scoreAfterTurn: turnScore, isCompleted: completed };
