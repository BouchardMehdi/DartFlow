...
export function applyThrow(history: GameHistory, dart: DartThrow): GameHistory {
  ...
  const state = history.present.modeState.kind === "x01" ? registerX01Throw(history.present, dart).state : history.present.modeState.kind === "around-the-clock" ? registerAroundTheClockThrow(history.present, dart).state : history.present.modeState.kind === "shanghai" ? registerShanghaiThrow(history.present, dart).state : history.present.modeState.kind === "cricket" ? registerCricketThrow(history.present, dart).state : registerKillerThrow(history.present, dart).state;
