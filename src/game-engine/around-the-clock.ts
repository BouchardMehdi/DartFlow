...
export function createAroundTheClockGame(players: Player[], progressionRule: AroundTheClockProgressionRule, bullFinish: boolean, maxRounds: number | null, direction: "ascending" | "descending" = "ascending", now = new Date()): GameState {
  ...
  const firstTarget = direction === "ascending" ? 1 : 20;
  return { id: crypto.randomUUID(), modeId: "around-the-clock", status: "in-progress", players: ordered, currentPlayerIndex: 0, currentRound: 1, currentTurn: makeTurn(first, 1, firstTarget, date), turns: [], modeState: { kind: "around-the-clock", progressionRule, direction, bullFinish, maxRounds, players: Object.fromEntries(ordered.map((player) => [player.id, { target: firstTarget, completed: false }])) }, createdAt: date, updatedAt: date };
}
...
export function registerAroundTheClockThrow(state: GameState, dart: DartThrow, now = new Date()): EngineResult {
  ...
  let won = false;
  if (targetHit) {
    const passedEnd = state.modeState.direction === "ascending" ? playerState.target + steps > 20 : playerState.target - steps < 1;
    if (playerState.target === 21 || (!state.modeState.bullFinish && passedEnd)) won = true;
    else if (passedEnd) target = 21;
    else target = state.modeState.direction === "ascending" ? playerState.target + steps : playerState.target - steps;
  }
  const darts = [...state.currentTurn.darts, dart]; const completed = darts.length === 3 || won;
  const turn: Turn = { ...state.currentTurn, darts, scoreAfterTurn: target, turnScore: state.currentTurn.turnScore + (targetHit ? Math.max(1, steps) : 0), isCompleted: completed };
  ...
  if (last && state.modeState.maxRounds !== null && state.currentRound === state.modeState.maxRounds) {
    const winner = [...state.players].sort((a, b) => state.modeState.kind === "around-the-clock" && state.modeState.direction === "descending" ? (modePlayers[a.id]?.target ?? 20) - (modePlayers[b.id]?.target ?? 20) || a.order - b.order : (modePlayers[b.id]?.target ?? 1) - (modePlayers[a.id]?.target ?? 1) || a.order - b.order)[0]; if (!winner) throw new Error("Gagnant introuvable");
  ...
  const nextIndex = last ? 0 : state.currentPlayerIndex + 1; const nextRound = last ? state.currentRound + 1 : state.currentRound; const next = state.players[nextIndex]; if (!next) throw new Error("Joueur suivant introuvable"); const nextTarget = modePlayers[next.id]?.target ?? (state.modeState.direction === "ascending" ? 1 : 20);
